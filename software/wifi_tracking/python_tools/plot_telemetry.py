...
Payload layout:
    ...
    Per subcarrier (num_subcarriers times):
        ...
        v_real[Nr x Nc]      Nr*Nc x float32 LE  row-major (steering matrix V)
...
class CBFPlotter:
    ...
    def _on_pick(self, event):
        ...
        for line, k in self._track_lines:
            line.set_alpha(
                _HIDDEN_ALPHA if k in self._hidden_streams else _VISIBLE_ALPHA)
        self.fig.canvas.draw_idle()

    def _register_stream_legend(self, leg, num_stream_entries):
        """
        Register the first num_stream_entries legend proxy lines as stream
        toggles. Remaining proxies (antenna linestyle entries) are read-only.
        """
        ...
        for i in range(min(num_stream_entries, len(proxies))):
            ...
            proxy.set_alpha(_HIDDEN_ALPHA if i in self._hidden_streams
                            else _VISIBLE_ALPHA)
    ...
    def _draw_mac_panel(self, snap, mac_idx):
        """
        For the selected MAC: plot each CBF steering vector element's complex
        value across every subcarrier.

        Color:     spatial stream index k  (COLORS[k])
        Linestyle: receive antenna index j  (LINESTYLES[j])
        ...
        for k in range(nc):
            a     = _HIDDEN_ALPHA if k in self._hidden_streams else _VISIBLE_ALPHA
    ...
    def _draw_right(self, snap, sc):
        ...
        if all_handles:
            leg = ax.legend(handles=all_handles,
                            ...
                            fontsize=7, loc='upper right')
            stream_proxies = leg.get_lines()[:len(stream_legend)]
            for k, proxy in enumerate(stream_proxies):
                proxy.set_picker(5)
                self._lmap[id(proxy)] = ('evec', k)
                proxy.set_alpha(_HIDDEN_ALPHA if k in self._hidden_streams
                                else _VISIBLE_ALPHA)
