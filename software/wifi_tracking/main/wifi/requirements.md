...
scan_for_cbf - This callback/function should filter for action and action no ack frames that contain the compressed beamforming matrix. This callback needs to be able to filter VHT beamforming packets, HE beamforming packets. This function should return some sort of data structure that provides info about the mac address, the sounding dialog token, the receive timestamp, SNR per spatial stream information, and the givens rotation angles per subcarrier. Feel free to include any other information that you believe may be useful for estimating the channel state. 
